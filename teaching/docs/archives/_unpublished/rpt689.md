...
\mathbf{e}_t &\sim \mathcal{N}(0, 0.1) \\
\mathbf{w}_t &\sim \mathcal{N}(0, 0.04) \\
\mathbf{x}_0 &\sim \mathcal{N}(0, 1) \quad \text{initial state,}
\end{aligned}
$$

The parameter vector then becomes $(\beta, \tau^2, \sigma^2) = (1, [0, 0.9], 0.1, 0.04')$. The simulated states are shown in Fig. 1(a) and the observations in Fig. 1(b). In all the algorithms, 1000 sample paths were generated. In running-window algorithms, a window of size 15 is chosen. Regular FFBS was used in the burn-in period in all the cases. While implementing Algorithms 2-3, we assumed the noise variances to be known to allow easy implementation. The algorithms are implemented in MATLAB® and the code is provided in the Appendix.
...
*[Figure 1: Data simulated from an AR(1) plus noise model. (a) states; (b) observations]*

*[Figure 2: estimated states from different algorithms (with fixed parameters)]*

*[Figure 3: estimated parameters from Algo-2b. (a) $\beta_0$; (b) $\beta_1$]*

*[Figure 4: estimated states from algo-2]*

*[Figure 5: CIs for parameters in Algo-3. (a) CIs for $\beta_0$; (b) CIs for $\beta_1$]*

*[Figure 6: estimates from different algorithms. (a) Algo-2; (b) All the Algorithms]*
